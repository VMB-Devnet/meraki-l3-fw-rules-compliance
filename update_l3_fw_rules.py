...
def main():
    ...
    if not org_id or len(org_id) > 1:
        logger.error(f"1 organization match not found for: {args.org}. Found: {org_id}. Exiting...")
        sys.exit(1)
    ...
    template_choice_id = [template['id'] for template in all_org_templates if template['name'] == args.template]
    
    if not template_choice_id:
        logger.error(f"Compliant Template: {args.template} found in Org: {args.org}. Exiting...")
        sys.exit(1)
    ...
    for template in template_to_configure:
        ...
        if template not in [template['name'] for template in all_org_templates]:
            logger.error(f"Destination Template: {template} not found in Org: {args.org}. Exiting...")
            sys.exit(1)
    ...
    templates_to_compare = [template for template in all_org_templates if template['name'] in template_to_configure]
    
    main_fw_rules = meraki_api.appliance.getNetworkApplianceFirewallL3FirewallRules(networkId=template_choice_id)

    for template in templates_to_compare:
        template['updated'] = False
        template['rules'] = meraki_api.appliance.getNetworkApplianceFirewallL3FirewallRules(networkId=template['id'])
    ...
    if not len([template for template in templates_to_compare if not template.get('rules_in_compliance')]) > 0:
    ...
    for template in templates_to_compare:
        if template['rules_in_compliance']:
            continue
        try:
            meraki_api.appliance.updateNetworkApplianceFirewallL3FirewallRules(networkId=template['id'], rules=main_fw_rules['rules'])
    ...
    templates_not_updated = [template for template in templates_to_compare if not template['updated']]
...
if __name__ == "__main__":
    ...
    if not api_key:
        raise TypeError("MERAKI_API_KEY not defined as Env variable..."
    ...
    parser.add_argument('templates_to_configure', help='Comma separated string of template names to make compliant')
    
    parser.add_argument('-v', help='Ask User for verification before updating FW rules. Defaults to False',  action='store_true')
